...
public class SampleDAO {
	...
	public static ContactList getContacts() {
		...
		//(현재 페이지0, 페이지 사이즈0, 목록사이즈 contacts.size(), List<Contact> contacts)
	...
	public static void deleteContact(long no) {
		...
		for(int i=0; i< contacts.size(); i++) {
			...
			//연락처의 일련번호가 전달받은 일련번호화 같을 때 목록에서 제거
			if(c.getNo() == no) {
				contacts.remove(i);
				count++;
				break;
